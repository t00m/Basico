...
class GtkAppWindow(BasicoWidget, Gtk.ApplicationWindow):
    ...
    def setup_headerbar(self):
        ...
        hb.props.title = "%s %s" % (APP['short'].title(), APP['version'])
        hb.props.subtitle = "Basico Notes Manager"
    ...
    def setup_headerbar_left(self, hb):
        ...
        # ~ self.srvgui.add_signal('gtk_button_visor_sapnotes', 'clicked', 'self.srvclb.display_about')
        lhbox.pack_start(button, False, False, 0)

        return lhbox

    def setup_headerbar_right(self, hb):
        rhbox = self.srvgui.add_widget('gtk_hbox_hb_right', Gtk.HBox())
